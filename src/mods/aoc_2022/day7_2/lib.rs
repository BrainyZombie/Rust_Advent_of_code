...
{
    const USABLE_SIZE: usize = 70000000 - 30000000;

    let root_node = create_node_tree(input);
    let used_size = root_node.size;
    let needed_size = used_size - USABLE_SIZE;
    // root_node.print(0);
    let top_dirs = get_top_dirs(&root_node, needed_size);

    let smallest_dir = top_dirs
        .iter()
        .min_by(|dir1, dir2| dir1.size.cmp(&dir2.size))
        .unwrap();

    Ok(smallest_dir.size.to_string())
}
