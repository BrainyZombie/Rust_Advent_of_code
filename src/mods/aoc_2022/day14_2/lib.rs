    ...
    T: Iterator<Item = String>,
{
    ...
    let mut paths: Vec<Vec<(usize, usize)>> = input
        .lines()
        .map(|path_string| {
            path_string
                .split(" -> ")
        ...
        .collect();
    paths.push(vec![
        (x_bounds.0 + 300, y_bounds.0 + 2),
        (x_bounds.1 - 300, y_bounds.0 + 2),
    ]);

    let x_bounds = (x_bounds.0 + 300, x_bounds.1 - 300);
    let y_bounds = (y_bounds.0 + 2, y_bounds.1);

    let mut grid = create_grid(&paths, (x_bounds.1, x_bounds.0, y_bounds.1, y_bounds.0));

    let FillResult::Filled(result) = dfs(&mut grid, (500-x_bounds.1 as isize, 0)) else {panic!()};
