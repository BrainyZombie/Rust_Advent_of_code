...
fn ranges_total_len(ranges: &[(isize, isize)], existing_beacons: &HashSet<isize>) -> usize {
    ranges.iter().fold(0, |acc, range| {
        acc + (range.1 - range.0 + 1) as usize
            - existing_beacons.iter().fold(0, |acc, x| {
    ...
    T: Iterator<Item = String>,
{
    ...
    let result = ranges_total_len(&ranges, &existing_beacons);
