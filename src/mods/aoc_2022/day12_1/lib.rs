...
fn bfs(lines: &Vec<&[u8]>) -> usize {
    ...
    loop {
        ...
        if c.1 > 0 {
            let u = (c.0, c.1 - 1);
            if !shortest_paths[u.0][u.1] && swap_letter(lines[u.0][u.1]) <= c_letter + 1 {
        ...
        if c.1 < lines[0].len() - 1 {
            let d = (c.0, c.1 + 1);
            if !shortest_paths[d.0][d.1] && swap_letter(lines[d.0][d.1]) <= c_letter + 1 {
        ...
        if c.0 > 0 {
            let l = (c.0 - 1, c.1);
            if !shortest_paths[l.0][l.1] && swap_letter(lines[l.0][l.1]) <= c_letter + 1 {
        ...
        if c.0 < lines.len() - 1 {
            let r = (c.0 + 1, c.1);
            if !shortest_paths[r.0][r.1] && swap_letter(lines[r.0][r.1]) <= c_letter + 1 {
